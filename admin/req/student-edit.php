...
if (isset($_POST['fname'])      &&
    ...
    if (empty($fname)) {
        ...
        $sql = "UPDATE students SET
                username = ?, fname=?, lname=?, department=?, address=?,gender = ?, section=?, email_address=?, date_of_birth=?, parent_fname=?,parent_lname=?,parent_phone_number=?, semester=?
                WHERE student_id=?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$uname,$fname, $lname, $department, $address, $gender,$section, $email_address, $date_of_birth, $parent_fname, $parent_lname,$parent_phone_number,$semester, $student_id]);
